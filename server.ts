...
app.post('/api/youtube/playlists/import', async (req, res) => {
  ...
  try {
    ...
    const dbVideos = ytVideos.map(video => ({
      id: video.id,
      title: video.title,
      video_url: video.videoUrl || `https://www.youtube.com/watch?v=${video.id}`,
    ...
    const { error: plUpdateErr } = await supabaseAdmin
      ...
      .update({
        ...
        lectures_count: ytVideos.length,
    ...
    const { error: logErr } = await supabaseAdmin.from('sync_logs').insert({
      ...
      channel_id: channelId,
      playlists_imported: 0,
      videos_imported: ytVideos.length,
      quota_used: estimateQuota('playlist_items', ytVideos.length),
      status: 'success',
      error_message: null,
      timestamp: new Date().toISOString()
    });
