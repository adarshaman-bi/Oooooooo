...
export async function fetchTeachers(filters?: {
  ...
  try {
    ...
    console.error('Supabase fetchTeachers error:', error);
    return [];
  }
}
...
export async function fetchTeacherById(id: string): Promise<TeacherProfile | null> {
  ...
  try {
    ...
    console.error(`Supabase fetchTeacherById error [${id}]:`, error);
    return null;
  }
}
...
export async function fetchInstitutes(): Promise<InstituteProfile[]> {
  try {
    ...
    return (data || []).map((inst: any) => ({
      ...
      trustScore: inst.trust_score || 90,
      followersCount: inst.followers_count || 0,
      officialLinks: inst.official_links || [],
      exams: inst.exams || ['JEE', 'NEET'],
      isVerified: inst.is_verified ?? true,
      createdAt: inst.created_at || new Date().toISOString()
    }));
  } catch (error) {
    console.error('Supabase fetchInstitutes error:', error);
    return [];
  }
}
...
export async function fetchInstituteById(id: string): Promise<InstituteProfile | null> {
  ...
  try {
    ...
    return {
      ...
      exams: data.exams || ['JEE', 'NEET'],
      isVerified: data.is_verified ?? true,
      createdAt: data.created_at || new Date().toISOString()
    };
  } catch (error) {
    console.error(`Supabase fetchInstituteById error [${id}]:`, error);
    return null;
  }
}
...
export async function fetchLectures(filters?: {
  ...
  try {
    ...
    // Apply strict content quality filters
    ...
    console.error('Supabase fetchLectures error:', error);
    return [];
  }
}
