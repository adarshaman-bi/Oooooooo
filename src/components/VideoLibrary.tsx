...
import { formatSubscribers } from '../utils/youtubeUtils';
...
export default function VideoLibrary({ onBackToHome }: VideoLibraryProps) {
  ...
  // Load Playlists & Channels on mount from Supabase
  useEffect(() => {
    setIsLoadingFeed(true);
    
    // Fetch channels from Supabase
    supabase.from('channels').select('*').then(({ data, error }) => {
      if (error) {
        console.error("Error loading channels:", error);
      } else {
        const items = (data || []).map((ch: any) => ({
          id: ch.id,
          channelId: ch.id,
          channelName: ch.name || '',
