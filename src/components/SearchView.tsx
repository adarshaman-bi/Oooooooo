...
interface SearchViewProps {
  ...
  setCurrentView: (v: 'explore' | 'profile' | 'moderator' | 'notifications' | 'search' | 'admin-educators' | 'teacher-detail') => void;
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {matchedPlaylists.map(p => (
                    <div key={p.id} className="bg-[#0D0D0C] border border-[#1A1A1A] hover:bg-zinc-900/45 rounded-2xl p-4 flex flex-col justify-between text-left gap-3 transition-colors">
                      <div className="space-y-1">
                        <h5 className="text-xs font-bold text-[#FFFFFF] uppercase tracking-tight line-clamp-1">{p.title}</h5>
                        <p className="text-[10px] text-zinc-400 leading-relaxed line-clamp-2">{p.description}</p>
                      </div>
                      <div className="pt-2 flex justify-between items-center text-[10px] text-zinc-500 font-sans">
