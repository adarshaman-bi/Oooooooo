...
router.get('/channel/:channelId', async (req, res) => {
  ...
  // Try fetching from local database first to serve fresh data and save API quota
  try {
    ...
    if (dbChannel) {
      ...
      if (dbPlaylists && dbPlaylists.length > 0) {
        const logo = dbChannel.avatar || '';
        const exams = dbChannel.exams || {};
        const banner = exams.bannerUrl || '';
        const subscriberCount = parseInt(dbChannel.subscribers || '0', 10) || 0;
        const viewCount = exams.viewCount || 0;
        const videoCount = exams.totalVideos || 0;
        const title = dbChannel.name || 'YouTube Channel';
        ...
        const channelData = {
          id: channelId,
          title,
          customName: exams.channelHandle || dbChannel.website || title,
          logo,
          banner,
          subscriberCount,
          viewCount,
        ...
        return res.json({ status: 'ok', source: 'database', data: channelData });
      }
    }
  } catch (dbErr: any) {
    console.warn(`[CHANNEL DB CHECK] Failed for ${channelId}:`, dbErr.message);
  }
...
router.get('/lectures/:playlistId', async (req, res) => {
  ...
  // Check if we have this channel/playlist videos in database first
  try {
    const isChannel = playlistId.startsWith('UC');
    const { data: dbVideos } = isChannel
      ? await supabaseAdmin
          .from('videos')
          .select('*')
    ...
    if (dbVideos && dbVideos.length > 0) {
      const formatted = dbVideos.map(v => ({
        ...
        publishedAt: v.published_at || v.publish_date || '',
    ...
    console.warn(`[LECTURES DB CHECK] Failed for ${playlistId}:`, dbErr.message);
  }

  // If it's a channel ID and not in DB, construct the uploads playlist ID for live API fallback
