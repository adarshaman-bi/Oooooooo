...
async function main() {
  console.log('=== PHASE 2: LIVE LECTURE EXTRACTION (NEXT 4 CHANNELS) ===\n');
  
  // Get next batch of active channels (skip first 3 already processed in Phase 1)
  const { data: channels, error: chError } = await supabase
    .from('channels')
    .select('id, name, avatar')
    .eq('is_active', true)
    .range(3, 6); // Phase 2: Next 4 channels
  ...
  console.log(`Processing ${channels.length} channels in Phase 2...\n`);
